...
# 11. Implemented Soft Stall Protection

The selected implementation follows the preferred Option A / external supervisory approach:

* hard current chopping remains in `bldc.c`
* autogenerated `BLDC_controller.c` diagnostics remain unchanged
* command derating is applied in `main.c` after normal command mixing and before the final `pwml` / `pwmr` outputs are written
* reusable derating state and math live in `stall_protection.c`

This keeps the soft operational-stall layer outside the FOC control loops while still reducing sustained stress before the autogenerated diagnostics are likely to force `OPEN_MODE`.

## 11.1 Exact diagnostic path confirmed

The current autogenerated diagnostic condition is:

```text
invalid hall state 000/111
OR
motor enabled + measured speed below n_stdStillDet + command above r_errInpTgtThres
```

The standstill branch is implemented in `F02_Diagnostics / Diagnostics_Enabled` as:

```c
rtb_RelationalOperator1_mv = (rtU->b_motEna &&
                              (Abs5 < rtP->n_stdStillDet) &&
                              (rtb_Saturation1 > rtP->r_errInpTgtThres));
```

That condition is ORed with invalid Hall states into a bitmask:

```text
bit 0: Hall 000
bit 1: Hall 111
bit 2: standstill/high-command latch
```

The bitmask is debounced by `t_errQual` / `t_errDequal`. On debounce edge, it updates `z_errCode`. The mode manager then forces `OPEN_MODE` whenever `Merge_p` is active:

```text
condition bitmask != 0
→ Debounce_Filter(..., t_errQual, t_errDequal, Merge_p, ...)
→ z_errCode
→ Merge_p
...
Current default relevant constants observed in `BLDC_controller_data.c` are:

* `t_errQual = 1280`
* `t_errDequal = 9600`
* `n_stdStillDet = 48`
* `r_errInpTgtThres = 9600`

Because `r_inpTgt` uses fixdt scaling internally, `r_errInpTgtThres = 9600` corresponds to roughly `600` command units at the external `[-1000, 1000]` command scale.

## 11.2 Implemented timeline

Soft stall derating is active when all of these are true for each motor independently:

* absolute mixed motor command is greater than `STALL_PROTECTION_CMD_THRESHOLD` (`650` by default)
* absolute measured motor speed is below `STALL_PROTECTION_SPEED_THRESHOLD_RPM` (`30 rpm` by default)
* `STALL_PROTECTION_ENABLE` is defined

The implemented timeline is:

```text
0-1000 ms     : requested command is passed through unchanged
1000-2000 ms  : command ceiling ramps linearly from 1000 to 350
>2000 ms      : command is clamped to +/-350 while the stall condition persists
```

If the wheel speed recovers or the command falls below the threshold, the per-motor stall timer resets immediately and the requested command is passed through again.

## 11.3 Safety-layer coordination

The soft threshold is intentionally set slightly above the internal diagnostic command threshold estimate:

```text
internal diagnostic threshold ≈ 600 command units
soft stall threshold          = 650 command units
```

This means ordinary low-speed manoeuvring below approximately `650` command units is not derated. High-command standstill events receive the full 1000 ms grace period and then ramp toward a sustained 350 command. Hard electrical current chopping and existing motor diagnostics are still allowed to override this behavior at any time.

## 11.4 Simulation notes

A discrete-time simulation at the configured `DELAY_IN_MAIN_LOOP = 5 ms` confirms the expected behavior for a constant `1000` command and `0 rpm` measured speed:

```text
t=0 ms     output=1000
t=1000 ms  output=1000
t=1500 ms  output=675
t=2000 ms  output=350
t>2000 ms  output=350
```

A second simulated case with speed recovery above `30 rpm` verifies that the stall timer resets and output immediately returns to the requested command.
