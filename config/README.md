...
- `control_tuning/velocity_setpoint_tuning.h`: longitudinal speed-intent mapping, setpoint-shaper tuning (`LONG_*`, physical `SETPOINT_ACCEL_*`/`SETPOINT_JERK_*`, slip `SETPOINT_SLIP_*`), and Step-D slip-aware soft-limit settings (`SOFT_LIMIT_*`).
