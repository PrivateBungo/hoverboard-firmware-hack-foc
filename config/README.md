...
- `control_tuning/velocity_setpoint_tuning.h`: setpoint-shaper tuning (physical `SETPOINT_ACCEL_*`/`SETPOINT_JERK_*`), slip `SETPOINT_SLIP_*`, and slip-aware soft-limit setting (`SOFT_LIMIT_*`).
- `control_tuning/motor_controller_gains.h`: outer velocity-controller (speed-domain PI) gains/limits used by `core/control/motor_controller.*`; distinct from generated high-frequency current-loop gains.
