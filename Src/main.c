...
static StallProtectionState stallStateL;
static StallProtectionState stallStateR;
...
int main(void) {
  ...
  while(1) {
    ...
    calcAvgSpeed();                       // Calculate average measured speed: speedAvg, speedAvgAbs
      ...
      // ####### SOFT STALL PROTECTION #######
      // Keep hard protections in bldc.c/autogenerated diagnostics intact, but reduce
      // sustained high torque when a wheel remains nearly stationary.
      cmdL = StallProtection_Update(&stallStateL, cmdL, rtY_Left.n_mot, DELAY_IN_MAIN_LOOP);
      cmdR = StallProtection_Update(&stallStateR, cmdR, rtY_Right.n_mot, DELAY_IN_MAIN_LOOP);
