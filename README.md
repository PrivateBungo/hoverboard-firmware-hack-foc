...
- Step E is complete and successful: in TRQ mode, an outer velocity PI controller now maps `v_sp` to torque request in the main loop (with saturation + anti-windup), replacing the temporary gain-only speed-to-torque path while preserving non-TRQ behavior.
