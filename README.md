...
- Step E is complete and successful: TRQ-mode longitudinal mapping now uses an outer velocity PI controller in the main loop (`v_sp` to torque request), with fixed-point anti-windup and explicit saturation/integrator telemetry (`vPI`, `iTerm`, `vSat`) for deterministic tuning and verification.
